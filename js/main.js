...
let paused = false;
let focusId = null;
let solar, planetarium, starfield, deepSpaceTex, demos, tour, blackhole;
...
async function boot() {
  ...
  await step(84, 'กำลังเตรียมระบบการเรียนรู้ภาษาไทย…', () => {
    blackhole = new BlackHole(scene, onPick);
  ...
  if (params.get('mode') === 'bh') setMode('bh');
...
function setMode(next) {
  ...
  blackhole.setVisible(isBH);
  starfield.visible = isSolar || isBH;
  scene.background = (isSolar || isBH) ? deepSpaceTex : new THREE.Color(0x010309);
  planetarium.setVisible(next === 'sky');
  $('deck-solar').classList.toggle('hidden', !isSolar);
  $('deck-sky').classList.toggle('hidden', next !== 'sky');
  $('compass').classList.toggle('hidden', next !== 'sky');
  $('deck-hint').textContent = isSolar
    ? 'คลิกดาวเคราะห์เพื่อดูข้อมูล · ลากเพื่อหมุน · ลูกกลิ้งเพื่อซูม'
    : isBH
      ? 'คลิกส่วนต่าง ๆ ของหลุมดำเพื่อเรียนรู้ · ลากเพื่อหมุนรอบ · ลูกกลิ้งเพื่อซูม'
      : 'ลากเพื่อกวาดมองท้องฟ้า · คลิกชื่อหมู่ดาวหรือดาวเพื่อดูข้อมูล';
  ui.mode = next;
  ui.fillDrawer(next, onPick);
  if (isSolar) applySolarCamera();
  else if (isBH) applyBHCamera();
  else applySkyCamera();
}

/* ── กล้องโหมดหลุมดำ ──────────────────────────────────────── */
function applyBHCamera() {
  controls.enabled = true;
  ...
  camera.fov = 55;
  camera.updateProjectionMatrix();
  // มุมมองเห็นทั้งกาแล็กซีล้อมหลุมดำ — ซูมเข้าไปดูใจกลางได้
  camera.position.set(10, 55, 130);
  controls.target.set(0, 0, 0);
}
...
renderer.domElement.addEventListener('pointerup', (e) => {
  ...
  downPos = null;
  if (moved > 6 || (mode !== 'solar' && mode !== 'bh')) return;
  pointer.set((e.clientX / innerWidth) * 2 - 1, -(e.clientY / innerHeight) * 2 + 1);
  raycaster.setFromCamera(pointer, camera);
  const targets = mode === 'bh'
    ? blackhole.pickables
    : [...solar.pickables, solar.beltPick];
